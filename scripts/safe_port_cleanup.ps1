# ============================================
# 安全端口清理工具 (Safe Port Cleanup Tool)
# D-68 决策：跨项目端口保护机制
# ============================================
#
# 规则：
...
Write-Host "  安全端口清理工具 (D-68)" -ForegroundColor Cyan
...
Write-Host "【正在扫描端口和进程】" -ForegroundColor Cyan
...
if ($allNodeProcs.Count -eq 0) {
    Write-Host "  未发现Node.exe进程" -ForegroundColor Green
    Write-Host ""
...
Write-Host ""
Write-Host "【Node.exe进程分析】" -ForegroundColor Cyan
...
if ($ownProcesses.Count -gt 0) {
    ...
    foreach ($p in $ownProcesses) {
        ...
        Write-Host "  PID $($p.PID) ($portsStr) - 内存 $($p.Memory)MB" -ForegroundColor Red
    }
}
...
if ($otherProcesses.Count -gt 0) {
    Write-Host ""
    Write-Host "【其他项目进程 - 仅警告，不清理】" -ForegroundColor Yellow
    foreach ($p in $otherProcesses) {
        $portsStr = if ($p.Ports.Count -gt 0) { "端口 $($p.Ports -join ', ')" } else { "无监听端口" }
        $projName = $projectPortRanges[$p.BelongsTo].Name
        Write-Host "  PID $($p.PID) [$projName] ($portsStr) - 内存 $($p.Memory)MB" -ForegroundColor Yellow
    }
}
...
if ($unknownProcesses.Count -gt 0) {
    ...
    foreach ($p in $unknownProcesses) {
        $portsStr = if ($p.Ports.Count -gt 0) { "端口 $($p.Ports -join ', ')" } else { "无监听端口" }
        Write-Host "  PID $($p.PID) ($portsStr) - 内存 $($p.Memory)MB" -ForegroundColor Magenta
        if ($p.CommandLine) {
...
Write-Host "  其他项目进程: $($otherProcesses.Count) 个 (保护)" -ForegroundColor Green
...
if ($ownProcesses.Count -eq 0) {
    ...
    if ($otherProcesses.Count -gt 0 -or $unknownProcesses.Count -gt 0) {
        Write-Host "发现其他项目/未知进程正在运行，已自动保护" -ForegroundColor Yellow
    }
    exit 0
}
...
foreach ($p in $ownProcesses) {
    try {
        ...
        Write-Host "  已清理 PID $($p.PID)" -ForegroundColor Green
        $successCount++
    } catch {
        Write-Host "  清理失败 PID $($p.PID) - $($_.Exception.Message)" -ForegroundColor Red
        $failCount++
    }
}
...
if ($remainingOwn.Count -gt 0) {
    Write-Host ""
    Write-Host "警告 - 以下进程清理失败，仍在运行" -ForegroundColor Yellow
...
if ($otherProcesses.Count -gt 0) {
    Write-Host ""
    Write-Host "其他项目进程已保护，仍在运行" -ForegroundColor Green
    foreach ($p in $otherProcesses) {
        $projName = $projectPortRanges[$p.BelongsTo].Name
