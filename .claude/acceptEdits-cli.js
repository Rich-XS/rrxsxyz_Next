...
function doBackup() {
  ...
  const backupDir = parentDir; // store zip directly under parent
  ...
  const keywords = sanitizeKeywords(process.argv[3] || '');
  ...
  function copyRecursive(src, dest) {
    for (const name of fs.readdirSync(src)) {
      ...
      if (rel.startsWith('Backup') || rel.includes('node_modules') || rel.includes('.git') || rel.startsWith('.claude\\Backup')) continue;
...
function writeStatusFile(state) {
  try { fs.writeFileSync(statusPath, state, 'utf8'); } catch (e) { console.error('Failed to write status file:', e.message); }
}
...
function updateProgressStatus(state) {
  try {
    ...
    if (idx === -1) idx = Math.min(5, lines.length - 1);
...
function showUsageAndExit() {
  console.log('Usage: node .claude/acceptEdits-cli.js on|off');
  process.exit(1);
}
...
(async () => {
  ...
  switch (mode) {
    case 'ON':
    case 'FULL':
      const confirmed = await confirmAction(`Are you sure you want to set AcceptEdits to ${mode}?`);
      if (!confirmed) {
    ...
    case 'TASK':
      ...
      break;
    default:
      console.log('Usage: node acceptEdits-cli.js [ON|OFF|FULL|TASK|STATUS]');
  }
})();
