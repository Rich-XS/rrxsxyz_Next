...
|---------|--------|------------|------|------|------|------------------|
| 第一性原理专家 | First Principle Expert | first_principle | FP | 蓝色核心层 | ⚛️ | 前提假设检验机制 |
| 时间穿越专家 | Time Traveler Expert | time_traveller | TT | 蓝色核心层 | ⏳ | 时序判断流程 |
| 上帝视角专家 | God View Expert | god_view | GV | 蓝色核心层 | 🌟 | 全局权衡模型 |
| 杠精专家 | Contrarian Expert | contrarian | CT | 紫色外部层 | ⚠️ | 批判性思维 |
| 买单客户 | Customer Expert | customer | CB | 紫色外部层 | 👤 | 业务反馈机制 |
| 竞争友商 | Competitor Expert | competitor | CP | 紫色外部层 | 📊 | 市场竞争模型 |
| 领袖(委托代理) | Delegate Expert | delegate | DL | 紫色外部层 | 🏆 | 任务分割机制 |
| 落地执行者 | Executor Expert | executor | EX | 绿色价值层 | 🛠️ | 实施负荷评估 |
| 风险管理者 | Risk Manager Expert | risk_manager | RM | 绿色价值层 | 🛡️ | 潜在风险预测 |
| 性能追求者 | Performance Expert | performance | PP | 绿色价值层 | ⚡ | 效率效能模型 |
| 情绪追求者 | Emotion Expert | emotion | EM | 绿色价值层 | 🫶 | 情绪倾向分析 |
| 社会认同者 | Social Expert | social | SA | 绿色价值层 | 👥 | 群体认知模型 |
| 潜在威胁 | Threat Expert | threat | TH | 绿色价值层 | ⚠️ | 不稳定因素识别 |
| 开发协调者 | Coordinator Expert | coordinator | CD | 绿色价值层 | 🆘 | 协作度评估 |
| 时间穿越者 | Time Revisitor Expert | time_revisitor | TR | 绿色价值层 | 🔄 | 承诺一致性 |
...
**1. 领袖开场要求**
- 展示主题和议程规划（必须提及关键问题）
- 具体描述（必须说明1-2个关键领域、约定规则）
- 提问式互动（"委托人，请补充简要"）
- 计划信号指明（是否共识触发某事五分钟队列）
- 语言简洁度：背景信息不超过500字符

**2. 委托人补充机制**
- 内容引导（"你觉得这个背景如何应用？"）
- 高度相关（回应领袖开场阐述内容）
- 信息采样（每轮补充内容控制在300-600字符范围内）

**3. 专家轮次发言结构**
- 每轮**轮流发言**：按照编码顺序播放
- 专家发言格式严格规定（标识标签后具体表达）
- 按照回合进行对话数控制（避免连续超时）
- 发言总控制在250-350字符，思路展开当不超过3段

**4. 中场转场导向**
- 小结表达：澄清道理、提及关键知识点
- 指向补充：初次明白陈述+个人归纳
- 动态调整：应答流反馈控制访客30秒内回复
...
- **语音与文字同步**：
  - 文字输出时确保不与前一段语音冲突
  - 张口后出现layın器（最快1.0s）开始语音标签+文字性别检测
- **语音播放策略**：
  - 固定延迟500ms（语音关闭）
  - 等待语音播放完成（语音打开）
