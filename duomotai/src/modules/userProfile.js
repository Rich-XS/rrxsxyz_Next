...
class UserProfile {
  constructor(config = {}) {
    this.config = {
      apiEndpoint: config.apiEndpoint || 'http://localhost:3001/api/user/profile',
  ...
  async showSurveyModal(phone) {
    return new Promise((resolve, reject) => {
      ...
      form.addEventListener('submit', async (e) => {
        ...
        try {
          // 保存到服务器（支持离线模式）
          const result = await this.saveToServer(profile);
          ...
          // 关闭模态框
          const modal = document.getElementById('profileSurveyModal');
          if (modal) {
            modal.remove();
          }
          ...
          if (result.source === 'local') {
            console.log('✅ 用户画像已保存到本地（离线模式）');
          }

          resolve(profile);

        } catch (error) {
          console.error('❌ 保存用户画像出错:', error);
          ...
          // 移除模态框，避免UI卡死（修复T-316：确保模态框被移除）
          const modal = document.getElementById('profileSurveyModal');
          if (modal) {
            modal.remove();
          }

          // 显示友好提示（修复T-316：改进错误提示）
          ...
          errorNotice.innerHTML = `
            ...
            ">
              <div style="display: flex; align-items: center; gap: 10px;">
                ...
                <div>
                  <strong>离线模式</strong><br/>
                  <span style="font-size: 0.9em;">
                    数据已保存到本地，将在网络恢复后自动同步
                  </span>
                </div>
              </div>
            </div>
          ...
          setTimeout(() => errorNotice.remove(), 5000); // 延长显示时间到5秒

          // 不要 reject，而是正常 resolve（允许用户继续使用）
          resolve(profile);
        }
      });
    });
  ...
  async saveToServer(profile) {
    try {
      ...
      console.warn('⚠️ 后端保存失败，使用本地存储:', error.message);
      ...
      // 返回成功（本地保存）而不是抛出错误
      return { success: true, source: 'local', message: '已保存到本地存储' };
    }
  }
  ...
  addToRetryQueue(profile) {
    try {
      ...
      localStorage.setItem('userProfile_retryQueue', JSON.stringify(queue));
      ...
      console.warn('⚠️ 添加到重试队列失败:', error);
    }
  }
      ...
      if (queue.length > 0 && navigator.onLine) {
        for (let i = queue.length - 1; i >= 0; i--) {
          ...
          try {
            ...
            // 网络错误：保留在队列中
            console.warn('⚠️ 重试失败，将继续重试:', error.message);
          }
        }
