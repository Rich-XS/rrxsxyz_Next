...
class UserDataService {
  ...
  async getUserProfile(phone) {
    try {
      ...
      const profileFileName = `${phone}_profile.json`;
      const profilePath = path.join(this.userDataDir, profileFileName);

      try {
        const content = await fs.readFile(profilePath, 'utf8');
        const profile = JSON.parse(content);
        console.log(`✅ [Task #044] 用户画像已加载: ${phone}`);
        return profile;
      } catch (error) {
        // Profile not found
        console.log(`⚠️ [Task #044] 用户画像不存在: ${phone}`);
        return null;
      }

    } catch (error) {
      console.error('❌ [Task #044] 读取用户画像失败:', error);
  ...
  async saveUserProfile(profile) {
    try {
      ...
      const profileFileName = `${profile.phone}_profile.json`;
      const profilePath = path.join(this.userDataDir, profileFileName);

      try {
        const existingContent = await fs.readFile(profilePath, 'utf8');
        const existingProfile = JSON.parse(existingContent);
        profileData.createdAt = existingProfile.createdAt; // Preserve original creation time
      } catch (error) {
        // Profile doesn't exist, use new createdAt
      }

      await fs.writeFile(profilePath, JSON.stringify(profileData, null, 2), 'utf8');
      console.log(`✅ [Task #044] 用户画像已保存: ${profileFileName}`);

      // Update summary table with profile info
      await this.updateSummaryWithProfile(profile);

      return profileData;

    } catch (error) {
      console.error('❌ [Task #044] 保存用户画像失败:', error);
      throw new Error(`保存用户画像失败: ${error.message}`);
    }
  }

  // ✅ [Task #044] 更新汇总表中的画像信息
  async updateSummaryWithProfile(profile) {
    try {
      let summaryData = {};
      ...
      try {
        ...
        // Summary file doesn't exist, create new one
        summaryData = {
          metadata: {
            created: new Date().toISOString(),
            totalUsers: 0,
      ...
      summaryData.users[userId].email = profile.email;
      summaryData.users[userId].nickname = profile.nickname;
      summaryData.users[userId].ageGroup = profile.ageGroup;
      summaryData.users[userId].gender = profile.gender;
      summaryData.users[userId].hasProfile = true;
      summaryData.users[userId].profileUpdatedAt = profile.updatedAt || new Date().toISOString();
      summaryData.users[userId].lastActivity = new Date().toISOString();
      ...
      // Save summary table
      await fs.writeFile(this.summaryFile, JSON.stringify(summaryData, null, 2), 'utf8');
      console.log(`✅ [Task #044] 汇总表已更新: 用户画像 (${profile.phone})`);

    } catch (error) {
      console.error('❌ [Task #044] 更新汇总表失败:', error);
