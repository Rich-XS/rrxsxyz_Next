  ...
  async initializeTransporter() {
    ...
    try {
      if (service.toLowerCase() === 'sendgrid') {
        ...
        // Other services (QQ, 163, etc.)
        this.transporter = nodemailer.createTransport({
          service: service,
          auth: {
            user: process.env.EMAIL_USER,
            pass: process.env.EMAIL_PASS // Authorization code
          }
        });
      }
...
如果您认为此邮件对您有帮助，欢迎分享给有需要的朋友`;
  ...
  markdownToHTML(markdown) {
    ...
    return markdown
      ...
      // Paragraphs (handle lists wrapped in <p> tags by splitting on double newline)
      .split('\n\n')
      .map(paragraph => {
        // If it doesn't start with an HTML tag, wrap it in <p>
        if (!paragraph.trim().startsWith('<') && paragraph.trim()) {
          // Handle lists (if split preserved list structure)
  ...
  // Generate email attachments (optional)
  generateAttachments(userName, reportContent) {
    // Logic for adding PDF or other resources goes here
    return [];
  }
...
// Export an asynchronous factory function instead of a direct instance
module.exports = {
  EmailService,
  createEmailService: async () => {
    const service = new EmailService();
    await service.initializeTransporter();
    if (!service.transporter) {
      throw new Error('Could not create EmailService instance: Transporter failed to initialize.');
    }
    return service;
  }
};